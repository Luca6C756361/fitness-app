...
import type { MuscleGroup } from "../_lib/types";

/**
 * Sagoma anatomica fronte/retro in stile "cyber-mesh": griglia tecnica sulla
 * silhouette, fasci muscolari colorati dinamicamente in base a un colore per
 * MuscleGroup passato via prop. Componente primitivo e agnostico rispetto al
 * significato del colore — usato sia da stats/MuscleRecoveryCard (verde/
 * giallo/rosso = stato di recupero) sia da ExerciseDetailModal (rosso pieno
 * = muscolo primario, ambra = secondario).
 *
 * Vive in today/_components (non in stats/_components) perché è condiviso
 * tra le due feature: stats lo importa da qui, esattamente come già fa per
 * i tipi/helper di today/_lib.
 *
 * Stessa architettura di MuscleMapSvg.tsx: componente unico, dichiarativo,
 * un solo SVG inline, nessun asset esterno — qui però i 10 MuscleGroup
 * "coarse" (non i 28 MuscleAnatomyId fini dell'atlante esercizi). Le
 * coordinate delle regioni sono derivate raggruppando quelle di
 * MuscleMapSvg — stesso "omino", stessa scala — solo lette in stile
 * scuro/neon invece che rosso su chiaro.
 */

interface BodyRecoveryMapProps {
  /** Colore hex per ciascun gruppo muscolare da evidenziare. */
  colorByMuscle: Partial<Record<MuscleGroup, string>>;
  className?: string;
  /** Descrizione per screen reader: personalizzabile in base al contesto d'uso. */
  srLabel?: string;
}
...
function FigureView({
  ...
  return (
    <svg viewBox="0 0 200 390" className="h-52 w-auto sm:h-60">
      ...
      {/* Fasci muscolari: colore + glow, dichiarativo. */}
      <g>
        {regions.map((r) => {
...
export default function BodyRecoveryMap({
  colorByMuscle,
  className = "",
  srLabel = "Mappa del corpo con i gruppi muscolari evidenziati, elencati in dettaglio sotto.",
