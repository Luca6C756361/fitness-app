    ...
    >
      ...
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold uppercase tracking-wide text-[#111111]/50">
              Esercizio {index + 1}
            </span>
            {isFullyDone && (
              <span className="flex items-center gap-1 rounded-full bg-teal-100 px-2 py-0.5 text-[10px] font-bold text-[#111111]">
                <CheckCircle2 className="h-3 w-3" /> Completo
              </span>
            )}
          </div>
          <h3 className="text-base font-bold text-[#111111]">{name}</h3>
          <p className="text-xs font-medium text-[#111111]/50 tabular-nums">
      ...
      {!isFullyDone && (
        <div className="rounded-xl border border-emerald-900/10 bg-white p-3">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs font-bold uppercase tracking-wide text-[#111111]/70">
          ...
          {mode === "simple" ? (
            <div>
              <label className="mb-1 block text-[10px] font-bold uppercase tracking-wide text-[#111111]/60">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Kg"
                  className="w-full rounded-lg border border-emerald-900/10 bg-white px-3 py-2 text-sm text-[#111111] tabular-nums outline-none focus:ring-2 focus:ring-emerald-300"
                />
                <button
                  type="button"
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                <label className="text-[10px] font-bold uppercase tracking-wide text-[#111111]/60">
              ...
              <div className="mb-2 grid grid-cols-4 gap-1.5 sm:grid-cols-6">
                {repWeights.map((w, i) => (
                  <div key={i} className="text-center">
                    <span className="mb-0.5 block text-[9px] font-bold uppercase text-[#111111]/40">
                      #{i + 1}
                    </span>
                    <input
                      type="number"
                      ...
                      placeholder="kg"
                      className="w-full rounded-md border border-emerald-900/10 bg-white px-1 py-1.5 text-center text-xs text-[#111111] tabular-nums outline-none focus:ring-2 focus:ring-emerald-300"
                    />
                  </div>
                ))}
              </div>
      ...
      {completedSets.length > 0 && (
        <button
          type="button"
          onClick={onUndo}
          className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg border border-emerald-900/10 bg-white/60 py-1.5 text-xs font-medium text-[#111111]/60 transition hover:bg-white hover:text-[#111111]"
        >
